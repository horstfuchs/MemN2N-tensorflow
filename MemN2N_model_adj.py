...
class MemN2N:
  def __init__(self, dictionary_size, memory_size, batch_size=32, embedding_dimensions=20,
    ...
    """
    print("USING: Adjacent weight tying. \n")
    ...
    with tf.name_scope("Embedding_Matrices"):
      if not self.embedding_lookup:
        self.emb_matrices = tf.Variable(tf.random_normal([self.number_hops + 1, 1, d, V], stddev=std_deviation), name="AC")
      else:
        # the first word in the dictionary is the nil word
        nil_word_slot = tf.zeros([self.number_hops + 1, 1, 1, d])
        random_initialization = tf.random_normal([self.number_hops + 1, 1, V-1, d], stddev=std_deviation)
      ...
      # self.emb_matrices = tf.Variable(tf.random_normal([self.number_hops + 1, self.batch_size, d, V], stddev=stdDeviation), name="AC")

    # Initialization of the temporal matrices
    with tf.name_scope("Temporal_Matrices"):
      if not self.embedding_lookup:
        self.temporal_matrices = tf.Variable(tf.random_normal([self.number_hops + 1, 1, d, M], stddev=std_deviation), name="T_AC")
      else:
        self.temporal_matrices = tf.Variable(tf.random_normal([self.number_hops + 1, 1, M, d], stddev=std_deviation), name="T_AC")
      # self.temporal_matrices = tf.Variable(tf.random_normal([self.number_hops + 1, 1, d, 1], stddev=stdDeviation), name="T_AC")
      # self.time_matrix_cut = tf.placeholder(tf.float32, shape=[None, 1, M])
  ...
  def buildGraph(self):
    ...
    with tf.name_scope("Embedding"):
      B = self.emb_matrices[0, 0]

      if not self.embedding_lookup:
        W = tf.transpose(self.emb_matrices[self.number_hops, 0], name="weights")
        u = tf.matmul(B, self.q, transpose_b=True, name="embeddedQuestion")
      else:
        W = self.emb_matrices[self.number_hops, 0]
    ...
    for hop in range(self.number_hops):
      with tf.name_scope("Embedding"):
        ...
        if not self.embedding_lookup:
          A_batches = tf.tile(self.emb_matrices[hop], (self.batch_size, 1, 1), name="A_batches")
          C_batches = tf.tile(self.emb_matrices[hop + 1], (self.batch_size, 1, 1), name="C_batches")

          m = tf.batch_matmul(A_batches, self.x, name="embeddedSentencesForInput") + self.temporal_matrices[hop]
          c = tf.batch_matmul(C_batches, self.x, name="embeddedSentencesForOutput") + self.temporal_matrices[hop+1]

        else:
          a_emb = tf.nn.embedding_lookup(self.emb_matrices[hop, 0], self.x, name="embeddingLookup_A")
          if self.position_encoding:
          ...
          m = tf.reduce_sum(a_emb, 2) + self.temporal_matrices[hop, 0]
          m = tf.transpose(m, (0, 2, 1), name="memory")

          c_emb = tf.nn.embedding_lookup(self.emb_matrices[hop+1, 0], self.x, name="embeddingLookup_C")
          if self.position_encoding:
            position_encoding = tf.tile(tf.reshape(self.position_encoding_matrix, (1, 1, self.max_sentence_length, self.embedding_dimensions)), (self.batch_size, self.memory_size, 1, 1))
            c_emb = tf.mul(position_encoding, c_emb, name="positionEncoding_C")
            # c_emb = self.position_encoding_matrix * c_emb
          c = tf.reduce_sum(c_emb, 2) + self.temporal_matrices[hop+1, 0]
      ...
      with tf.name_scope("SummationPrediction"):
        ...
        u = tf.add(tf.transpose(o_2d), u, name="sum_u")
  ...
  def saveTensorBoardImages(self, epoch, train_writer, feed_dictionary):
    ...
    """
    if not self.embedding_lookup:
      emb_matrices_img = self.processTensorBoardColourImage(tf.reshape(self.emb_matrices, (self.number_hops + 1, self.embedding_dimensions, self.dictionary_size, 1)))
    else:
      emb_matrices_img = self.processTensorBoardColourImage(tf.reshape(self.emb_matrices, (self.number_hops + 1, self.dictionary_size, self.embedding_dimensions, 1)))
    ...
    if not self.embedding_lookup:
      temp_matrices_img = self.processTensorBoardColourImage(tf.reshape(self.temporal_matrices[:, :, :, 0:memories_to_print], (self.number_hops + 1, self.embedding_dimensions, memories_to_print, 1)))
    else:
      temp_matrices_img = self.processTensorBoardColourImage(tf.reshape(self.temporal_matrices[:, :, 0:memories_to_print], (self.number_hops + 1, memories_to_print, self.embedding_dimensions, 1)))
    ...
    gradient_img = self.processTensorBoardColourImage(tf.reshape(self.clipped_grads_and_vars[0][0], (self.number_hops + 1, self.dictionary_size, self.embedding_dimensions, 1)))
  ...
  def printProbsWithSentences(self, batch_for_print, answers, data, step, feed_dictionary):
    ...
    if self.number_hops >= 3:
      output_predicted_answers, p0_val, p1_val, p2_val = self.sess.run([self.predicted_answer, self.p_0, self.p_1, self.p_2], feed_dict=feed_dictionary)
    if self.number_hops == 2:
      output_predicted_answers, p0_val, p1_val = self.sess.run([self.predicted_answer, self.p_0, self.p_1], feed_dict=feed_dictionary)
    if self.number_hops == 1:
      output_predicted_answers, p0_val = self.sess.run([self.predicted_answer, self.p_0], feed_dict=feed_dictionary)
